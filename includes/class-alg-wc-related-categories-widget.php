...
if ( ! class_exists( 'Alg_WC_Related_Categories_Widget' ) ) :
	...
	/**
	 * Alg_WC_Related_Categories_Widget class.
	 *
	 * @version 2.1.0
	 * @since   1.7.0
	 */
	class Alg_WC_Related_Categories_Widget extends WP_Widget {
		 ...
		 */
		public function get_related_categories( $instance ) {
		...
		public function widget( $args, $instance ) {
			$related_categories = $this->get_related_categories( $instance );
			if ( $related_categories ) {
				...
				echo wp_kses(
					$html,
					alg_wc_related_categories()->core->frontend->get_allowed_html()
				);
			}
		}

		/**
		 * Get widget option fields.
		 *
		 * @version 2.1.0
		 * @since   1.7.0
		 *
		 ...
		 */
		public function get_widget_option_fields() {
			$settings = array(
				// General Options.
				'title'      => array(
			...
			$settings = array_merge(
				...
				array(
					...
					'template_custom'        => array(
						...
						'desc'    => '<details>' .
							'<summary>' .
								__( 'Available placeholders', 'related-categories-for-woocommerce' ) .
							'</summary>' .
							'<p>' .
		 ...
		 */
		public function get_allowed_options_html() {
			$allowed_html = array(
				'input'  => array(
					'type'    => true,
					'id'      => true,
		...
		public function form( $instance ) {
			...
			foreach ( $this->get_widget_option_fields() as $id => $widget_option_field ) {
				if (
					isset( $widget_option_field['type'] ) &&
					'title' === $widget_option_field['type']
				) {
					$html .= '<hr><p><strong>' . $widget_option_field['title'] . '</strong></p>';
					continue;
				}
				$value = ( $instance[ $id ] ?? $widget_option_field['default'] );
				$label = sprintf(
					'<label for="%s">%s</label>',
					$this->get_field_id( $id ),
					$widget_option_field['title']
				);
				...
				$desc        = (
					...
					'<br><em>' . $widget_option_field['desc'] . '</em>' :
					''
				);
				$custom_atts = (
