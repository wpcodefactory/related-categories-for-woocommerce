...
if ( ! class_exists( 'Alg_WC_Related_Categories_Settings_General' ) ) :
	...
	/**
	 * Alg_WC_Related_Categories_Settings_General class.
	 *
	 * @version 2.1.0
	 * @since   1.7.0
	 */
	class Alg_WC_Related_Categories_Settings_General extends Alg_WC_Related_Categories_Settings_Section {
		 ...
		 * @since   1.7.0
		 *
		 * @param string $single_or_loop Indicates whether the settings are for the single product page or the archives (loop) page.
		 *
		 * @todo (dev) Use `{$this->option_prefix}` instead of `alg_wc_related_categories_options_{$this->type}`.
		 */
		public function __construct( $single_or_loop ) {
			$this->id    = (
				'single' === $single_or_loop ?
				'' :
				'loop'
			);
			$this->desc  = (
				'single' === $single_or_loop ?
				__( 'Single', 'related-categories-for-woocommerce' ) :
				__( 'Archives', 'related-categories-for-woocommerce' )
			);
			$this->title = (
				'single' === $single_or_loop ?
				__( 'Single Product Page', 'related-categories-for-woocommerce' ) :
				__( 'Archives', 'related-categories-for-woocommerce' )
			);
		 ...
		 */
		public function get_relate_settings() {
				...
				array(
					...
					array(
						...
						'desc'  => __( 'These options will allow you to set related categories <strong>manually</strong>.', 'related-categories-for-woocommerce' ) .
							apply_filters(
								'alg_wc_related_categories_settings',
		 ...
		 */
		public function get_position_settings() {
		...
		public function get_template_settings() {
			...
			return array(
				...
				array(
					...
					'default' => '<section class="related categories">' . PHP_EOL .
						'<h2>' . __( 'Related categories', 'related-categories-for-woocommerce' ) . '</h2>',
					'type'    => 'textarea',
